...
import * as json from "../db/weapon_expert.json";
...
export class WeaponExpert extends ExpertTrader {
    ...
    public createAssortTable(): ITraderAssort {
        ...
        for (const key in this.tables.templates.items) {
            ...
            if (
                parent != ItemParent.AssaultCarbine &&
                parent != ItemParent.AssaultRifle &&
                parent != ItemParent.SniperRifle &&
                parent != ItemParent.GrenadeLauncher &&
                parent != ItemParent.MachineGun &&
                parent != ItemParent.MarksmanRifle &&
                parent != ItemParent.Knife &&
                parent != ItemParent.Pistol &&
                parent != ItemParent.Revolver &&
                parent != ItemParent.Shotgun &&
                parent != ItemParent.Smg &&
                parent != ItemParent.ThrowWeap
            ) continue;
