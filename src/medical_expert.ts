...
import * as json from "../db/medical_expert.json";
...
export class MedicalExpert extends ExpertTrader {
    ...
    public createAssortTable(): ITraderAssort {
        ...
        for (const key in this.tables.templates.items) {
            ...
            if (
                parent != ItemParent.Drink &&
                parent != ItemParent.Food &&
                parent != ItemParent.Stimulator &&
                parent != ItemParent.Medical &&
                parent != ItemParent.MedKit &&
                parent != ItemParent.Drugs &&
                parent != ItemParent.Keycard &&
                parent != ItemParent.KeyMechanical &&
                parent != ItemParent.PortableRangeFinder &&
                parent != ItemParent.RepairKits &&
                parent != ItemParent.Compass &&
                parent != ItemParent.Map
            ) continue;
