...
import * as json from "../db/barter_expert.json";
...
export class BarterExpert extends ExpertTrader {
    ...
    public createAssortTable(): ITraderAssort {
        ...
        for (const key in this.tables.templates.items) {
            ...
            if (
                parent != ItemParent.Battery &&
                parent != ItemParent.BuildingMaterial &&
                parent != ItemParent.Electronics &&
                parent != ItemParent.HouseholdGoods &&
                parent != ItemParent.Jewelry &&
                parent != ItemParent.Fuel &&
                parent != ItemParent.Lubricant &&
                parent != ItemParent.MedicalSupplies &&
                parent != ItemParent.Other &&
                parent != ItemParent.Tool
            ) continue;
